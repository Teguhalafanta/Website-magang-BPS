                        ...
                        <tbody>
                            @forelse ($kegiatans as $index => $kegiatan)
                                <tr id="kegiatan-{{ $kegiatan->id }}">
                                    ...
                                    <td class="tanggal text-center">{{ \Carbon\Carbon::parse($kegiatan->tanggal)->format('d-m-Y') }}</td>
                                    ...
                                    <td class="text-center durasi">{{ $kegiatan->durasi ? $kegiatan->durasi . ' mnt' : '-' }}</td>
                                    <td class="text-center status">
                                        <span class="badge bg-{{ $kegiatan->status_penyelesaian == 'Selesai' ? 'success' : ($kegiatan->status_penyelesaian == 'Dalam Proses' ? 'warning' : 'secondary') }}">
                ...
                <form id="formEditKegiatan">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row g-3">
                            <div class="col-md-8">
                                <label class="form-label">Nama Kegiatan <span class="text-danger">*</span></label>
                                <input type="text" name="nama_kegiatan" id="edit-nama_kegiatan" 
                            ...
                            <div class="col-md-6">
                                <label class="form-label">Pemberi Tugas</label>
                                <input type="text" name="pemberi_tugas" id="edit-pemberi_tugas" 
                                       class="form-control">
                            ...
                            <div class="col-md-12">
                                <label class="form-label">Tim Kerja</label>
                                <input type="text" name="tim_kerja" id="edit-tim_kerja" 
                                       class="form-control" placeholder="Anggota tim yang terlibat">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function clearErrors(prefix = '') {
        const fields = ['nama_kegiatan', 'tanggal', 'deskripsi', 'volume', 'satuan', 'durasi', 'pemberi_tugas', 'tim_kerja', 'status_penyelesaian'];
        ...
        fetch("{{ route('kegiatan.store') }}", {
            ...
            .then(res => {
                if (res.errors) {
                    ...
                    const statusBadgeClass = res.kegiatan.status_penyelesaian == 'Selesai' ? 'success' : 
                                           (res.kegiatan.status_penyelesaian == 'Dalam Proses' ? 'warning' : 'secondary');
        ...
        if (e.target.closest('.btnEdit')) {
            ...
            fetch(`/pelajar/kegiatan/${id}/edit`)
                ...
                .then(k => {
                    ...
                    document.getElementById('edit-status_penyelesaian').value = k.status_penyelesaian || '';
                ...
                fetch(`/pelajar/kegiatan/${id}`, {
                    ...
                    .then(r => {
                        if (r.success) {
                            ...
                            // Update nomor urut
                            document.querySelectorAll('#tableKegiatan tbody tr').forEach((tr, i) => {
            ...
            .then(res => {
                if (res.errors) {
                    ...
                    if (tr) {
                        ...
                        tr.querySelector('.tanggal').innerText = res.kegiatan.tanggal_formatted || res.kegiatan.tanggal;
                        tr.querySelector('.deskripsi').innerText = res.kegiatan.deskripsi.length > 50 ? 
                            res.kegiatan.deskripsi.substring(0, 50) + '...' : res.kegiatan.deskripsi;
                        tr.querySelector('.volume').innerText = res.kegiatan.volume || '-';
                        tr.querySelector('.satuan').innerText = res.kegiatan.satuan || '-';
                        tr.querySelector('.durasi').innerText = res.kegiatan.durasi ? res.kegiatan.durasi + ' mnt' : '-';
                        tr.querySelector('.status').innerHTML = `<span class="badge bg-${statusBadgeClass}">${res.kegiatan.status_penyelesaian}</span>`;
                    }
